...
export default {
  ...
  methods: {
    ...
    viewFormData (attachmentId, formTemplateId, attachedFormTemplateName, attachedFormTemplate) {
      ...
      */
      ...
      axios.get(process.env.VUE_APP_API_URL + get_url)
                     ...
                     if (field.type !== "table") {
                       ...
                       const tableHeaderColumnsWidthMap = {};
                             ...
                             tableSchemaData.forEach(columnHeader => {
                                tableHeaderColumns = tableHeaderColumns + `<div style='padding: 5px;max-width: 150px;min-width:${columnHeader.minWidth}'>${columnHeader.label}</div>`;
                       ...
                       let tableDataHTML = ``;
                            ...
                            tableData.forEach(row => {
                                ...
                                let columnString = ``;
                                row.forEach(col => {
                                    columnString = columnString + `<div style=';max-width: 150px;min-width:${tableHeaderColumnsWidthMap[col.name]};padding: 5px;'>${col.value}</div>`;
                                });
                        ...
                        const fieldHTML = `<div style='display:flex;'>
                                               <div style=";max-width: 150px;min-width:100px;text-align:right;padding:5px;font-weight:bold">${field.label}</div>
                                               <div style=";max-width: 150px;min-width:100px;text-align:left;;padding:5px;padding-left:20px">${tableHTML}</div>
                                           </div>`;
          ...
          .catch((error) => {
          UIkit.notification("<span uk-icon='icon: warning;ratio: 0.75'></span> viewFormData :: " + error, {
            status: 'danger',
            pos: 'bottom-left',
            timeout: 5000
          });
    ...
    getFormTemplateMetadata (formTemplateId) {
      ...
      if (this.activityId) {
        form = document.querySelector('[data-activity-container-id="' + this.activityId + '"]')
         ...
         form = document.querySelector('.edit_form_template_container');
      }
      form.innerHTML = '';
    ...
    loadTableInFormField (fieldInfo, dropzone) {
      ...
      const template = `<form autocomplete="off">
        <div style="display: flex;column-gap: 10px;background-color: #dccfcf;text-transform: none;font-weight: bold;letter-spacing: 1px;">
            <template v-for="item in tableColumnsArray">
                <div v-if="item.type==='number'" v-bind:style="'text-align: right;;max-width: 150px;min-width:' + item.minWidth + ';padding: 5px;'">{{item.label}}</div>
                <div v-else v-bind:style="';max-width: 150px;min-width:' + item.minWidth + ';padding: 5px;'">{{item.label}}</div>
            </template>
        </div>
      ...
      const columnTemplate = `<div v-if="item.type==='number'" v-bind:style="'text-align: right;max-width: 150px;min-width:' + item.minWidth + ';padding: 5px;'">{{item.label}}</div>
                              <div v-else v-bind:style="';max-width: 150px;min-width:' + item.minWidth + ';padding: 5px;'">{{item.label}}</div>`;
...

      {
        ...
        const availableColumnsInTable = this.attachmentInfo[fieldInfo["name"] + "TableSchema"];
              availableColumnsInTable.forEach(columnMetadata => {
                        ...
                        if (columnType === "integer" || columnType === "bigdecimal" || columnType === "number" || columnType === "float") {
                            columnHeaderTemplateHTML += `<div style="text-align: right;max-width: 150px;min-width:${columnMinWidth};padding: 5px;">${columnLabel}</div>`;
                        }
                        else {
                          columnHeaderTemplateHTML += `<div style=";max-width: 150px;min-width:${columnMinWidth};padding: 5px;">${columnLabel}</div>`;
                        }
        ...
        tableData.forEach((row, i) => {
            ...
            const rowElement = document.querySelector('[data-row-key="' + currentRowKey + '"');
            // console.log("row :: ", row);
    ...
    submitFormAttachment () {
      ...
      //                      document.querySelectorAll('.edit_form_template_container .form-input-element');
       ...
       if (this.activityId) {
         formContainer = document.querySelector('[data-activity-container-id="' + this.activityId + '"]')
          ...
          formContainer = document.querySelector('.edit_form_template_container');
       }
      ...
      const form = {
        id: this.attachmentDataId, // Since we only allow the form to be submitted once and not update data, we comment the below line and use only 'new' instead.
    ...
    // View Form Functions
    getFormAttachment (attachmentDataId) {

      // console.log('getFormAttachment called..' + attachmentDataId);
      try
      {
        const get_url = './task-service/task-form-attachments/get-form-data/' + attachmentDataId;
        // console.log("Fetching data for form from url " + get_url);
        axios.get(process.env.VUE_APP_API_URL + get_url)
          .then((dataResponse) => {
            ...
            if (dataResponse.data.actionResult == 1) {
              ...
              if (this.activityId) {
                form = document.querySelector('[data-activity-container-id="' + this.activityId + '"]')
                 ...
                 form = document.querySelector('.form_data_display_container');
              }
