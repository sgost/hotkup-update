...
<div>
    ...
    <div v-show="availableSubtasks.length>0" class="subtasks_box">
        ...
        <template v-for="(subtask, idx) in availableSubtasks" :key="idx">
...
export default {
    ...
    methods: {
        ...
        loadSubtasks() {
            try {
                const url = `https://test.hotkup.com/tasks/list-by-crm-client/${this.item.id}/1/all`; // Fetch all subtasks
        ...
        getUnpaginatedList(listKey, url) {
            //        console.log("Mixin : Retrieving data from 1: " + url);
        ...
        handleUnpaginatedListData(listKey, data) {
            if (listKey === 'subtasks') {
                this.handleFetchedSubtasks(data);
            } else if (listKey === 'dependency_subtasks') {
                this.handleFetchedDependencySubtasks(data);
            }
        },
        ...
        handleFetchedSubtasks(dataResponse) {
            ...
            const subTasks = dataResponse.data.data;
            ...
            // console.log('availableSubtasks', this.availableSubtasks)
        },
